...
void FourierFit(CanvasData* data) {
    data->cosfit_curve.clear();
    ...
    for (int i = 0; i < n; ++i) {
        ...
        float tmpxs = 0.f;
        float tmpys = 0.f;
        //{//TODO: 频率kf和频率mf的cos进行内积不等于0，不正交？
        //    for (int k = 0; k < n; ++k) {
        //        for (int m = 0; m < n; ++m) {
        //            float tmp = 0.f;
        //            //频率k*f 和 频率 m*f 进行内积求和
        //            for (int j = 0; j < n; ++j) {
        //                tmp += cos(2 * 3.1415926 * k * f * (j * dt)) * cos(2 * 3.1415926 * m * f * (j * dt));
        //            }
        //            cout << tmp << endl;
        //        }
        //    }
        //}
        for (int j = 0; j < n; ++j) {
            tmpxc += data->points[j].x * cos(2 * 3.1415926 * i * f * (j * dt));
            tmpyc += data->points[j].y * cos(2 * 3.1415926 * i * f * (j * dt));
            tmpxs += data->points[j].x * sin(2 * 3.1415926 * i * f * (j * dt));
            tmpys += data->points[j].y * sin(2 * 3.1415926 * i * f * (j * dt));
        }
        axc[i] = (float)2 / n * tmpxc;
        ayc[i] = (float)2 / n * tmpyc;
        axs[i] = (float)2 / n * tmpxs;
        ays[i] = (float)2 / n * tmpys;
    ...
    for (int i = 0; i < data->num; ++i) {
        float t = i * dt;
        float x = (axc[0]+axs[0]) / 2;
        float y = (ayc[0] + ays[0]) / 2;
        for (int j = 1; j < n; ++j) {
            x += axc[j] * cos(2 * 3.1415926 * j * f * t)+ axs[j] * sin(2 * 3.1415926 * j * f * t);
            y += ayc[j] * cos(2 * 3.1415926 * j * f * t) + ays[j] * sin(2 * 3.1415926 * j * f * t);
        }
        data->cosfit_curve.push_back(ImVec2(x, y));
    }
    data->cos_draw = true;

}

void CosinFit(CanvasData* data) {
    data->cosfit_curve.clear();
    ...
    for (int i = 0; i < n; ++i) {
        ...
        //}
        for (int j = 0; j < n; ++j) {
            tmpx += data->points[j].x * cos(2 * 3.1415926 *i *f * (j * dt));
            tmpy += data->points[j].y * cos(2 * 3.1415926 *i* f * (j * dt));
        }
        ax[i] = (float)2 / n * tmpx;
        ay[i] = (float)2 / n * tmpy;
    }
    ...
    for (int i = 0; i < data->num; ++i) {
        ...
        float y = ay[0]/2;
        for (int j = 1; j < n; ++j) {
            x += ax[j] * cos(2 * 3.1415926 * j * f * t);
            y += ay[j] * cos(2 * 3.1415926 * j * f * t);
        }
        data->cosfit_curve.push_back(ImVec2(x, y));
    }
    data->cos_draw = true;
...
int main() {
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        if (ImGui::Begin("Canvas")) {
            ...
            if (data.cos_draw && data.cosfit_curve.size() > 1) {
                for (int i = 0; i < data.cosfit_curve.size() - 1; ++i) {
                    draw_list->AddLine(ImVec2(origin + data.cosfit_curve[i]), ImVec2(origin + data.cosfit_curve[i + 1]), IM_COL32(255, 255, 0, 255));
                }
            }
